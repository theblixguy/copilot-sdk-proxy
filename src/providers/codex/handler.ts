...
export function createResponsesHandler(
  ...
  return async function handleResponses(
    ...
    let prompt: string;
    try {
      prompt = formatResponsesPrompt(slicedInput);
    ...
    try {
      if (options?.handleStreaming) {
        ...
        const healthy = await handleResponsesStreaming(conversation.session, prompt, req.model, reply, responseId, logger, stats);
        conversation.sessionActive = false;
        if (healthy) {
