...
describe("Claude provider", () => {
  ...
  it(
    "streams a basic response with Anthropic SSE events",
    async () => {
      ...
      const types = (parseSSELines(res.body) as { type?: string }[]).map(
        (e) => e.type,
      );
  ...
  it(
    ...
    async () => {
      const res = await postJSON(baseUrl, PATH, msg("think about life", 16000));
      expect(res.status).toBe(200);
