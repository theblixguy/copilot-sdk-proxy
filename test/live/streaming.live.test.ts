...
async function startServer(provider: Parameters<typeof createServer>[1]) {
  const ctx = createCtx();
...
async function postJSON(baseUrl: string, path: string, body: unknown): Promise<{ status: number; body: string }> {
...
describe.each(CODEX_MODELS)("Codex provider with %s", (model) => {
  ...
  it("streams a basic response", async () => {
    const res = await postJSON(baseUrl, "/v1/responses", { model, input: "Reply with exactly: hello" });
  ...
  it("rejects missing input", async () => {
    const res = await postJSON(baseUrl, "/v1/responses", { model });
