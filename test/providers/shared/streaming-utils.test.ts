...
function mockReply(): {
  ...
  } as unknown as Parameters<typeof sendSSEEvent>[0];
...
describe("formatCompaction", () => {
  ...
  it("returns unavailable when fields are missing", () => {
    expect(formatCompaction({ preCompactionTokens: 100 })).toBe(
      "compaction data unavailable",
    );
    expect(formatCompaction({ postCompactionTokens: 100 })).toBe(
      "compaction data unavailable",
    );
  });
});
