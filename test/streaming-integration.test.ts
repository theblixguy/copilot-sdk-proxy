...
import type { SessionEvent, SessionEventHandler, CopilotSession, SessionConfig } from "@github/copilot-sdk";
...
function createCtx(sequence: EventSequence): AppContext {
  return {
    service: {
      cwd: process.cwd(),
      createSession: (_config: SessionConfig) => Promise.resolve(createMockSession(sequence)),
...
function createMultiTurnCtx(sequences: EventSequence[]): AppContext {
  ...
  return {
    service: {
      cwd: process.cwd(),
      createSession: (_config: SessionConfig) => Promise.resolve(session),
      listModels: () => Promise.resolve([
        { id: "test-model", capabilities: { supports: { reasoningEffort: false } } },
      ]),
      ping: () => Promise.resolve({ message: "ok", timestamp: Date.now() }),
...
describe("OpenAI streaming integration", () => {
  ...
  it("handles session error", async () => {
    ...
    // Stream still completes (HTTP 200 was already sent), but no text content
    expect(res.statusCode).toBe(200);
  ...
  it("handles session error after partial deltas", async () => {
    ...
    expect(res.statusCode).toBe(200);
    // Partial deltas may or may not be flushed depending on timing,
    // but the stream should terminate without hanging
  });
