...
Every request goes through [`runHandlerPipeline()`](../src/providers/shared/handler-core.ts#L38). The pipeline is generic over the request type.
...
Each provider implements [`StreamProtocol`](../src/providers/shared/streaming-core.ts#L7), which defines how to flush text deltas, reasoning deltas, and completion/failure signals to the HTTP response.
...
[`createCommonEventHandler()`](../src/providers/shared/streaming-core.ts#L24) handles events shared across all three protocols. It buffers text and reasoning deltas, logs tool executions, records usage stats, and logs context compaction.
